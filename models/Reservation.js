...
class Reservation {
    ...
    static findByUser(iduser, time, date, hour) {
        if(time === 'previous') {
            const sql = 'SELECT idreservation, place, name, date, hour, partySize, status, imageFirst FROM reservation r JOIN place p ON p.idplace = r.place WHERE customer = ? AND date < ? UNION SELECT idreservation, place, name, date, hour, partySize, status, imageFirst FROM reservation r JOIN place p ON p.idplace = r.place WHERE customer = ? AND date = ? AND hour < ? ORDER BY date DESC, hour DESC;';
